...
<tr>
	...
	<td>
		<form action="/team5/ranking/fishLengthDetail.do" role="search" class="search-form">
			...
			<input type="hidden" name="fishName" value="${fishName}">
 ...
 <table>
      ...
      <tbody>
        	...
        	<c:if test="${not empty myrank.rank}">
        	    <tr class="myRanking">
          		<td><r>내 순위<r></td>
        	    <td>${myrank.rank}위        상위 ${Math.round(myrank.rank/cnt*100)}%</td>
          		<td>${myrank.fishLength}m</td>
          		<td>${myrank.fishWeight}cm</td>
        		</tr>
        	</c:if>
      </tbody>
    </table>
